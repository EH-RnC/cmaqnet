...
def get_unet_model(
    ...
    def encoder_block(x_map, x_met=None, t_emb=None, filters:int=1, kernel:int=3, dropout:float=0.0):
        ...
        if x_met is not None:
            x_met = tf.keras.layers.Dense(filters)(x_met)
            x_met = tf.keras.layers.Activation(activation)(x_met)
            x_met = tf.keras.layers.Reshape((1, 1, filters))(x_met)
    ...
    x = x0 = encoder_block(met_maps, ctrl_inputs, t_emb, in_filters, kernel, dropout)
    x = tf.keras.layers.MaxPool2D()(x)
    x = x1 = encoder_block(x, ctrl_inputs, t_emb, in_filters*2, kernel, dropout)
    x = tf.keras.layers.MaxPool2D()(x)
    x = x2 = encoder_block(x, ctrl_inputs, t_emb, in_filters*4, kernel, dropout)
    x = tf.keras.layers.MaxPool2D()(x)
    x = x3 = encoder_block(x, ctrl_inputs, t_emb, in_filters*8, kernel, dropout)
    x = tf.keras.layers.MaxPool2D()(x)
    
    x = encoder_block(x, ctrl_inputs, t_emb, in_filters*16, kernel, dropout)
    x = decoder_block(x, x3, ctrl_inputs, in_filters*8, kernel, dropout, t_emb)
    x = decoder_block(x, x2, ctrl_inputs, in_filters*4, kernel, dropout, t_emb)
    x = decoder_block(x, x1, ctrl_inputs, in_filters*2, kernel, dropout, t_emb)
    x = decoder_block(x, x0, ctrl_inputs, in_filters, kernel, dropout, t_emb)
