...
def get_unet_model(
    ...
    def attention_gate(x, g, filters:int=1):
        ...
        out = tf.keras.layers.Multiply()([g, psi])
        return out
    ...
    def encoder_block(x_map, x_met=None, t_emb=None, filters:int=1, kernel:int=3, dropout:float=0.0):
        ...
        if x_met is not None:
            x_met = tf.keras.layers.Conv2D(filters, kernel, padding='same')(x_met)
            x_met = tf.keras.layers.Activation(activation)(x_met)
            x_met = tf.keras.layers.Resizing(*x_map.shape[1:3])(x_met)
            x_param = tf.keras.layers.Multiply()([x_map, x_met])
    ...
    def decoder_block(x0, x1, x_met=None, filters:int=1, kernel:int=3, dropout:float=0.0, t_emb=None):
        ...
        if x_met is not None:
            x_met = tf.keras.layers.Conv2D(filters, kernel, padding='same')(x_met)
            x_met = tf.keras.layers.Activation(activation)(x_met)
            x_met = tf.keras.layers.Resizing(*x0.shape[1:3])(x_met)
            x_param = tf.keras.layers.Multiply()([x0, x_met])

        x1 = attention_gate(x0, x1, filters)
    ...
    met_inputs = tf.keras.Input(shape=met_input_shape)
    ...
    x = x0 = encoder_block(met_maps, ctrl_maps, t_emb, in_filters, kernel, dropout)
    x = tf.keras.layers.MaxPool2D()(x)
    x = x1 = encoder_block(x, ctrl_maps, t_emb, in_filters*2, kernel, dropout)
    x = tf.keras.layers.MaxPool2D()(x)
    x = x2 = encoder_block(x, ctrl_maps, t_emb, in_filters*4, kernel, dropout)
    x = tf.keras.layers.MaxPool2D()(x)
    x = x3 = encoder_block(x, ctrl_maps, t_emb, in_filters*8, kernel, dropout)
    x = tf.keras.layers.MaxPool2D()(x)
    
    x = encoder_block(x, ctrl_maps, t_emb, in_filters*16, kernel, dropout)
    x = decoder_block(x, x3, ctrl_maps, in_filters*8, kernel, dropout, t_emb)
    x = decoder_block(x, x2, ctrl_maps, in_filters*4, kernel, dropout, t_emb)
    x = decoder_block(x, x1, ctrl_maps, in_filters*2, kernel, dropout, t_emb)
    x = decoder_block(x, x0, ctrl_maps, in_filters, kernel, dropout, t_emb)

    # x = x0 = encoder_block(ctrl_maps, met_maps, t_emb, in_filters, kernel, dropout)
    # x = tf.keras.layers.MaxPool2D()(x)
    # x = x1 = encoder_block(x, met_maps, t_emb, in_filters*2, kernel, dropout)
    # x = tf.keras.layers.MaxPool2D()(x)
    # x = x2 = encoder_block(x, met_maps, t_emb, in_filters*4, kernel, dropout)
    # x = tf.keras.layers.MaxPool2D()(x)
    # x = x3 = encoder_block(x, met_maps, t_emb, in_filters*8, kernel, dropout)
    # x = tf.keras.layers.MaxPool2D()(x)
    
    # x = encoder_block(x, met_maps, t_emb, in_filters*16, kernel, dropout)
    # x = decoder_block(x, x3, met_maps, in_filters*8, kernel, dropout, t_emb)
    # x = decoder_block(x, x2, met_maps, in_filters*4, kernel, dropout, t_emb)
    # x = decoder_block(x, x1, met_maps, in_filters*2, kernel, dropout, t_emb)
    # x = decoder_block(x, x0, met_maps, in_filters, kernel, dropout, t_emb)
